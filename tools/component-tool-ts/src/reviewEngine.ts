...
export function buildReviewPrompt(
  ...
  const assumptions = component.outgoing_contracts.map(
    (c) =>
      `- **${c.contract_id}**: Depends on \`${c.target_component_id}\` — ${c.expectation}`,
  );

  const obligations = component.incoming_contracts.map(
    (c) =>
      `- **${c.contract_id}**: Expected by \`${c.source_component_id}\` — ${c.expectation}`,
  );
...
export class ReviewScheduler {
  pending: ReviewTask[] = [];
  inProgress: ReviewTask | null = null;
  ...
  enqueue(task: ReviewTask): boolean {
    ...
    if (existing === task.fingerprint) return false;
...
export function computeComponentFingerprint(
  ...
  const scopedFiles = resolveScope(
    component.scope,
    componentsRoot,
    null,
    gitRoot,
  ...
  return computeFingerprint(
    componentContent,
    fileHashes,
    component.incoming_contracts,
    component.outgoing_contracts,
  );
}

export function runSingleReview(
  component: Component,
  ...
  if (result.parsed_json) {
    try {
      const json = result.parsed_json as Record<string, unknown>;
      const issues = (
        (json.issues as Array<Record<string, unknown>>) || []
      ).map(
        (i): ReviewIssue => ({
          issue_id: (i.issue_id as number) || 0,
          severity: (i.severity as ReviewIssue["severity"]) || "low",
          kind: (i.kind as ReviewIssue["kind"]) || "other",
          title: (i.title as string) || "",
          details: (i.details as string) || "",
          evidence: ((i.evidence as Array<Record<string, string>>) || []).map(
            (e): Evidence => ({
              path: e.path || "",
              lines: e.lines || "",
            }),
          ),
          suggested_action: (i.suggested_action as string) || "",
        }),
      );

      const proposedInvariants = (
        (json.proposed_invariants as Array<Record<string, string>>) || []
      ).map(
        (i): ProposedInvariant => ({
      ...
      return {
        component_id: component.component_id,
        status: (json.status as ReviewStatus) || "ok",
        issues,
...
export function getChangedFiles(gitRoot: string): string[] {
...
export function resolveAffectedComponents(
  changedFiles: string[],
  graph: ComponentGraph,
  componentsRoot: string,
  gitRoot: string | null = null,
