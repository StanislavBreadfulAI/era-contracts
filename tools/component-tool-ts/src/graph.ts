...
export function resolveScope(
  ...
  if (scope.include.length === 0) return [];
...
export function buildGraph(
  ...
  const parsed = new Map<
    string,
    {
      frontmatter: ReturnType<typeof parseComponentFile>["frontmatter"];
      description: string;
      filePath: string;
    }
  >();
  ...
  for (const [cid, { frontmatter: fm, description, filePath }] of parsed) {
    ...
    for (const dep of fm.dependencies) {
      ...
      if (dep.target_path) {
        targetPath = resolveComponentPath(
          dep.target_path,
          filePath,
          componentsRoot,
        );
      ...
      if (!graph.dependencies_of[cid]) graph.dependencies_of[cid] = [];
    ...
    for (const dep of fm.dependants) {
      ...
      if (dep.source_path) {
        sourcePath = resolveComponentPath(
          dep.source_path,
          filePath,
          componentsRoot,
        );
      ...
      if (!graph.dependants_of[cid]) graph.dependants_of[cid] = [];
