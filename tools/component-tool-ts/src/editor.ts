...
export function addDependency(options: {
  ...
  // Parse both files
  const { frontmatter: sourceFm, description: sourceDesc } =
    parseComponentFile(sourceFile);
  const { frontmatter: targetFm, description: targetDesc } =
    parseComponentFile(targetFile);

  // Check for duplicates
  if (sourceFm.dependencies.some((d) => d.contract_id === contractId)) {
    throw new Error(
      `Contract '${contractId}' already exists in ${sourceFile}`,
    );
  }
  if (targetFm.dependants.some((d) => d.contract_id === contractId)) {
    throw new Error(
      `Contract '${contractId}' already exists in ${targetFile}`,
    );
  }
