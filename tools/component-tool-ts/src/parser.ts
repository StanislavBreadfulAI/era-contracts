...
export function parseComponentFile(
  filePath: string,
): { frontmatter: ComponentFrontmatter; description: string } {
...
export function frontmatterToYaml(fm: ComponentFrontmatter): string {
  ...
  if (fm.scope.include.length > 0 || fm.scope.exclude.length > 0) {
    ...
    if (fm.scope.include.length > 0) scope.include = fm.scope.include;
    if (fm.scope.exclude.length > 0) scope.exclude = fm.scope.exclude;
    data.scope = scope;
  }

  if (fm.dependencies.length > 0) {
    data.dependencies = fm.dependencies.map((dep) => {
      const d: Record<string, string> = { contract_id: dep.contract_id };
      if (dep.target_component_id) d.target_component_id = dep.target_component_id;
      if (dep.target_path) d.target_path = dep.target_path;
  ...
  if (fm.dependants.length > 0) {
    data.dependants = fm.dependants.map((dep) => {
      const d: Record<string, string> = { contract_id: dep.contract_id };
      if (dep.source_component_id) d.source_component_id = dep.source_component_id;
      if (dep.source_path) d.source_path = dep.source_path;
      d.expectation = dep.expectation;
      return d;
    });
  }
...
export function generateScopeSection(scope: ScopeSpec): string {
  const lines: string[] = [];
  for (const p of scope.include) lines.push(`- include: \`${p}\``);
  for (const p of scope.exclude) lines.push(`- exclude: \`${p}\``);
  return lines.length > 0 ? lines.join("\n") : "- (no scope defined)";
}

export function generateDependenciesSection(
...
export function generateDependantsSection(
  deps: DependencyEntry[],
): string {
  if (deps.length === 0) return "- (none)";
  return deps
