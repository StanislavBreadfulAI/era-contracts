...
export function validateGraph(graph, componentsRoot, gitRoot = null, allFiles = null) {
    const issues = [];
    // Check for duplicate contract_ids across components
    const seenContractIds = new Map();
    ...
    for (const [cid, comp] of Object.entries(graph.components)) {
        ...
        for (const contract of comp.outgoing_contracts) {
            ...
            const mirror = targetComp.incoming_contracts.find((c) => c.contract_id === contract.contract_id);
            if (!mirror) {
                issues.push({
                    severity: "error",
        ...
        for (const contract of comp.incoming_contracts) {
            ...
            const mirror = sourceComp.outgoing_contracts.find((c) => c.contract_id === contract.contract_id);
            if (!mirror) {
                issues.push({
                    severity: "error",
