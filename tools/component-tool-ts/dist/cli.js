...
import { ReviewScheduler, computeComponentFingerprint, getChangedFiles, resolveAffectedComponents, runSingleReview, } from "./reviewEngine.js";
import { findGitRoot } from "./scanner.js";
import { validateGraph } from "./validator.js";
const program = new Command();
program
    .name("component-tool")
    .description("CLI tool for managing and reviewing conceptual components described by .component.md files.")
    .version("0.1.0");
// ─── Helpers ────────────────────────────────────────────────────────────────
function resolveRoots(workspace) {
    const workspaceRoot = resolve(workspace);
    const gitRoot = findGitRoot(workspaceRoot);
    return { workspaceRoot, gitRoot, componentsRoot: workspaceRoot };
}
function loadAndValidate(componentsRoot, gitRoot, failOnError = true) {
...
function findComponent(graph, compId) {
    if (graph.components[compId])
        return compId;
    const matches = Object.keys(graph.components).filter((cid) => cid.startsWith(compId));
    if (matches.length === 1)
        return matches[0];
    if (matches.length > 1) {
        console.error(chalk.red(`Ambiguous component '${compId}'. Matches: ${matches.join(", ")}`));
        process.exit(1);
    }
    console.error(chalk.red(`Component '${compId}' not found.`));
    process.exit(1);
}
function makeAdapter(agent, model) {
    if (agent === "claude")
...
function runReviews(componentIds, graph, componentsRoot, gitRoot, adapter, timeout, runId) {
    ...
    while ((task = scheduler.next()) !== null) {
        ...
        const result = runSingleReview(comp, graph, componentsRoot, adapter, gitRoot, timeout);
        ...
        console.log(`  ${statusColor(result.status)} — ${result.issues.length} issue(s)`);
    }
    return run;
}
...
program
    ...
    for (const cid of ids) {
        ...
        if (comp.scope.include.length > 3)
            scopeSummary += "...";
        table.push([
            cid,
            comp.title,
...
program
    ...
    .description("Show outgoing contracts for a component, optionally filtered by target.")
    .option("-w, --workspace <path>", "Workspace root directory", ".")
    .option("--json", "Output as JSON")
    .action((comp1, comp2, opts) => {
    const { componentsRoot, gitRoot } = resolveRoots(opts.workspace);
    const graph = loadAndValidate(componentsRoot, gitRoot, false);
...
program
    ...
    const componentId = opts.id || absPath.split("/").pop().replace(".component.md", "").replace(/[/\\]/g, ".");
...
program
    ...
    .option("-w, --workspace <path>", "Workspace root directory", ".")
    .action((comp1, comp2, expectation, opts) => {
    const { componentsRoot, gitRoot } = resolveRoots(opts.workspace);
    ...
    try {
        ...
        console.log(chalk.green(`Added contract '${usedId}':`), `${cid1} -> ${cid2}`);
    }
    catch (err) {
        console.error(chalk.red(err.message));
        process.exit(1);
...
program
    ...
    .option("-o, --output <path>", "Output directory for reports", ".component-tool/reviews")
    .action((components, opts) => {
    const { componentsRoot, gitRoot } = resolveRoots(opts.workspace);
    const graph = loadAndValidate(componentsRoot, gitRoot, true);
    const adapter = makeAdapter(opts.agent, opts.model);
    const runId = getRunId();
    const resolvedIds = components.map((c) => findComponent(graph, c));
    const run = runReviews(resolvedIds, graph, componentsRoot, gitRoot, adapter, parseInt(opts.timeout), runId);
...
program
    ...
    if (!opts.yes) {
        console.error(chalk.bold.yellow("WARNING:"), `This will review ALL ${ids.length} components. ` +
    ...
    const adapter = makeAdapter(opts.agent, opts.model);
    const runId = getRunId();
    const run = runReviews(ids, graph, componentsRoot, gitRoot, adapter, parseInt(opts.timeout), runId);
    const absOutput = resolve(componentsRoot, opts.output);
    const runDir = writeReports(run, absOutput);
    console.log(chalk.green("\nReports written to:"), runDir);
    printSummary(run);
...
program
    ...
    const affected = resolveAffectedComponents(changedFiles, graph, componentsRoot, gitRoot);
    ...
    for (const f of changedFiles) {
        ...
        if (rel.startsWith(".."))
            continue;
        if (!graph.file_to_components[rel] &&
            !f.endsWith(".component.md")) {
            console.error(chalk.yellow("WARNING:"), `Changed file '${f}' belongs to no component`);
        }
    }
    if (affected.size === 0) {
        console.log(chalk.yellow("No components affected by current changes."));
        return;
    }
    ...
    const adapter = makeAdapter(opts.agent, opts.model);
    const runId = getRunId();
    const run = runReviews(sortedAffected, graph, componentsRoot, gitRoot, adapter, parseInt(opts.timeout), runId);
    const absOutput = resolve(componentsRoot, opts.output);
    const runDir = writeReports(run, absOutput);
    console.log(chalk.green("\nReports written to:"), runDir);
    printSummary(run);
